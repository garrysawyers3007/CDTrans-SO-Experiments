...
from datasets.make_dataloader import train_collate_fn, source_target_train_collate_fn, image_train_adain
from datasets.sampler import RandomIdentitySampler
from datasets.bases import ImageDataset, AugImageDataset, ImageList_style
...
def obtain_label(logger, val_loader, model, distance='cosine', threshold=0):
    ...
    print('obtain label')
    model.eval()
    for n_iter, (aug_img, aug_type, img, vid, _, _, _) in enumerate(tqdm(val_loader)):
        with torch.no_grad():
            img = img.to(device)
...
def update_feat(cfg, epoch, model, train_loader1,train_loader2, device,feat_memory1,feat_memory2, label_memory1,label_memory2):
    model.eval()
    for n_iter, (aug_img, aug_type, img, vid, _, _, idx) in enumerate(tqdm(train_loader1)):
        with torch.no_grad():
            img = img.to(device)
            feats = model(img, img)
    ...
    for n_iter, (aug_img, aug_type, img, vid, _, _, idx) in enumerate(tqdm(train_loader2)):
        with torch.no_grad():
            img = img.to(device)
            feats = model(img, img)
            feat = feats[1]/(torch.norm(feats[1],2,1,True)+1e-8)
...
def generate_new_dataset(cfg, logger, label_memory2, s_dataset, t_dataset, knnidx, target_knnidx, target_pseudo_label, label_memory1, img_num1, img_num2, with_pseudo_label_filter=True):
    ...
    new_dataset = AugImageDataset(train_set, train_transforms, adain_transform=image_train_adain(), style_path = './FDA_dataset_rendition/')
...
def do_train_uda(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, ((aug_img, aug_type), imgs, vid, target_cam, target_view, file_name, idx) in enumerate(train_loader):
            try:
            ...
            indx = (aug_type!=None).nonzero()

            idx_others = np.array(list((set(indx[0]) - set(idx_adain[0])) - set(idx_styleaug[0])))
            ...
            s_idx_adain = s_idx[idx_adain[0]]
            t_idx_adain = t_idx[idx_adain[0]]
            label_knn_adain = label_knn[idx_adain[0]]
            target_cam_adain = target_cam[idx_adain[0]]
            target_view_adain = target_view[idx_adain[0]]
            ...
            s_idx_styleaug = s_idx[idx_styleaug[0]]
            t_idx_styleaug = t_idx[idx_styleaug[0]]
            label_knn_styleaug = label_knn[idx_styleaug[0]]
            target_cam_styleaug = target_cam[idx_styleaug[0]]
            target_view_styleaug = target_view[idx_styleaug[0]]
            ...
            s_idx_others = s_idx[idx_others]
            t_idx_others = t_idx[idx_others]
            label_knn_others = label_knn[idx_others]
            target_cam_others = target_cam[idx_others]
            target_view_others = target_view[idx_others]
            ...
            try:
                with torch.no_grad():
                    ...
                    t_im_restyled_styleaug = augmentor(t_img_styleaug.cuda())
                ...
                s_idx = torch.cat((s_idx_others, s_idx_adain, s_idx_styleaug), dim=0)
                t_idx = torch.cat((t_idx_others, t_idx_adain, t_idx_styleaug), dim=0)
            ...
            img = img.to(device)
            t_img = t_img.to(device) #target img
            target = target.to(device)
            t_pseudo_target = t_pseudo_target.to(device)
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (aug_img, aug_type, img, vid, camid, camids, target_view, idex) in enumerate(val_loader):
                    with torch.no_grad():
                        img = img.to(device)
                        camids = camids.to(device)
                        target_view = target_view.to(device)
