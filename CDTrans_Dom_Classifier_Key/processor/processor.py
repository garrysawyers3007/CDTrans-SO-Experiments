...
def do_train_pretrain(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, layer_num, dom_cls=False):
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.train()
        for n_iter, (rand_img, img, vid, target_cam, target_view, _) in enumerate(train_loader):
            # print('aaaaaa!!!')
            rand_imgs = []
            ...
            W = int(img[0, 0].shape[0]/8)
            H = int(img[0, 0].shape[1]/8)
            ...
            final_rand_img = shuffle_batch(final_rand_img, 8)
            ...
            final_rand_img = final_rand_img.to(device)
...
def do_inference(cfg,
    ...
    model.eval()
    img_path_list = []
    for n_iter, (_, img, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            img = img.to(device)
            camids = camids.to(device)
            target_view = target_view.to(device)
