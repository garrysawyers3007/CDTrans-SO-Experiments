...
def train_collate_fn(batch):
    ...
    """
    rand_imgs, imgs, pids, camids, viewids , _ , idx= zip(*batch)
    # print('train collate fn' , imgs)
    pids = torch.tensor(pids, dtype=torch.int64)
    viewids = torch.tensor(viewids, dtype=torch.int64)
    camids = torch.tensor(camids, dtype=torch.int64)
    rand_imgs = [torch.stack(rand_img, dim=0) for rand_img in rand_imgs]
    return torch.stack(rand_imgs, dim=0), torch.stack(imgs, dim=0), pids, camids, viewids, idx

def val_collate_fn(batch):##### revised by luo
    rand_imgs, imgs, pids, camids, viewids, img_paths, idx = zip(*batch)
    viewids = torch.tensor(viewids, dtype=torch.int64)
...
def make_dataloader(cfg, args):
    ...
    train_set = ImageDataset(dataset.train, train_transforms, patch_size=args.patch_size)
    train_set_clean = ImageDataset(dataset.train, train_transforms)
    ...
    if cfg.DATASETS.QUERY_MINING:
        if cfg.MODEL.TASK_TYPE == 'classify_DA':
            ...
            val_set_shuffled = ImageDataset(dataset.test, val_transforms, patch_size=args.patch_size)
        else:
            val_set = ImageDataset(dataset.query + dataset.query, val_transforms)
            val_set_shuffled = ImageDataset(dataset.query + dataset.query, val_transforms, patch_size=args.patch_size)
    else:
        if cfg.MODEL.TASK_TYPE == 'classify_DA':
            val_set = ImageDataset(dataset.test, val_transforms)
            val_set_shuffled = ImageDataset(dataset.test, val_transforms, patch_size=args.patch_size)
            ...
            val_set_shuffled = ImageDataset(dataset.query + dataset.gallery, val_transforms, patch_size=args.patch_size)
