    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (aug_img, aug_type, img, feature, vid, target_cam, target_view, paths, _) in enumerate(train_loader):
            ...
            try:
                ...
                data_stylized = normalize(torch.cat([data_stylized_adain, aug_im_restyled_styleaug], 1))
            ...
            target_view = target_view.to(device)
            with amp.autocast(enabled=True):
                score, feat = model(aug_img, img, target, cam_label=target_cam, view_label=target_view, feature=feature )
                loss = loss_fn(score, feat, target, target_cam)
...
def do_train_pretrain_patch_augs(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, num_patch_wise, layer_num):
    log_period = cfg.SOLVER.LOG_PERIOD
    checkpoint_period = cfg.SOLVER.CHECKPOINT_PERIOD
    eval_period = cfg.SOLVER.EVAL_PERIOD
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (aug_img, aug_type, img, vid, target_cam, target_view, paths, _) in enumerate(train_loader):
            ...
            for l in range(num_patch_wise):
                ...
                for k in range(aug_img.shape[0]):
                    ...
                    for i in range(int(W)):
                        for j in range(int(H)):
                            ...
                            final_img[:, i*patch_size:i*patch_size+patch_size,j*patch_size:j*patch_size+patch_size] = aug_img[k, aug_type, :, i*patch_size:i*patch_size+patch_size, j*patch_size:j*patch_size+patch_size]
                ...
                with torch.no_grad():
                    feat = model_feature(final_patch_aug_img, final_patch_aug_img, cam_label=target_cam, view_label=target_view)
...
def do_inference_rare_class(cfg,
                 M1,
                 M2,
                 val_loader,
                 num_query):
    ...
    for n_iter, (aug_img, aug_type, img, feature, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            ...
            if cfg.TEST.EVAL:
                if cfg.MODEL.TASK_TYPE == 'classify_DA':
                    probs1 = M1(img, img, cam_label=camids, view_label=target_view, return_logits=True)
                    probs2 = M2(img, img, cam_label=camids, view_label=target_view, return_logits=True)
                    _, preds1 = torch.max(probs1, 1)
                    _, preds2 = torch.max(probs2, 1)

                    for t, p in zip(torch.tensor(pid).view(-1), preds1.view(-1)):
                        confusion_matrix1[t.long(), p.long()] += 1
                    for t, p in zip(torch.tensor(pid).view(-1), preds2.view(-1)):
                        confusion_matrix2[t.long(), p.long()] += 1

                    evaluator.update((probs1, pid))
                else:
                    feat1 = M1(img, img, cam_label=camids, view_label=target_view)
                    feat2 = M2(img, img, cam_label=camids, view_label=target_view)
                    evaluator.update((feat1, pid, camid))
            else:
                feat1 = M1(img, img, cam_label=camids, view_label=target_view)
                feat2 = M2(img, img, cam_label=camids, view_label=target_view)
                evaluator.update((feat1, pid, camid, target_view, imgpath))
            img_path_list.extend(imgpath)

    print("M1 per class acc:", confusion_matrix1.diag()/confusion_matrix1.sum(1))
    print("M2 per class acc:", confusion_matrix2.diag()/confusion_matrix2.sum(1))
    print("Per class numbers:", confusion_matrix2.sum(1))
