...
class Attention(nn.Module):
    ...
    def forward(self, aug_x, x):
        ...
        # qkv_aug = self.qkv(aug_x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # q, k, v = qkv[0], qkv_aug[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
...
class Block(nn.Module):
    ...
    def forward(self, aug_x,  x):
        a, key = self.attn(self.norm1(aug_x), self.norm1(x))
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        num_patches = self.patch_embed.num_patches
        self.dc_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.dc_pos_embed = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
    ...
    def forward_features(self, aug_x, x, camera_id, view_id, layer_num):
        B = x.shape[0]
        x = self.patch_embed(x)
        aug_x = self.patch_embed(aug_x)

        dc_tokens = self.dc_token.expand(B, -1, -1)
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((dc_tokens, cls_tokens, x), dim=1)
        aug_x = torch.cat((dc_tokens, cls_tokens, aug_x), dim=1)

        pos_embed = torch.cat((self.dc_pos_embed, self.pos_embed), dim=1)
        x = x + pos_embed
        x = self.pos_drop(x)

        aug_x = aug_x + pos_embed
        aug_x = self.pos_drop(aug_x)
    ...
    def forward(self, aug_x, x, cam_label=None, view_label=None, layer_num=0):
        x = self.forward_features(aug_x, x, cam_label, view_label, layer_num)
        return x

    def corrected_dict(self, state_dict):
        new_dict = {}

        for k,v in state_dict.items():
            if 'qkv' in k:
                new_dict[k.replace('qkv', 'q')] = v[:self.embed_dim]
                new_dict[k.replace('qkv', 'k')] = v[self.embed_dim:2*self.embed_dim]
                new_dict[k.replace('qkv', 'v')] = v[2*self.embed_dim:]
            else:
                new_dict[k] = v
        return new_dict
    ...
    def load_param(self, model_path):
        ...
        param_dict = self.corrected_dict(param_dict)
