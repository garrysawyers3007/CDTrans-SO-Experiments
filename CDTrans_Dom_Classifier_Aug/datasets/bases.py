...
class ImageDataset(Dataset):
    def __init__(self, dataset, transform=None, style_path=None, adain_transform=None, mode='RGB', patch_size=1, training=False):
    ...
    def get_aug_full_img(self, index, aug_index):
        ...
        rand_path, rand_pid, rand_camid, rand_trackid, rand_idx = self.dataset[index]
        
        style_img = self.loader(random.choice(self.style_list))

        if isinstance(path,tuple):
            img = self.loader(path[0])
            rand_img = self.loader(rand_path[0])
        else:
            img = self.loader(path)
            rand_img = self.loader(rand_path)
        ...
        #aug_type = aug_list[index%6] if self.training==False else random.choice(aug_list)
        ...
        if aug_type == 'weather':
            ...
            rand_img_path_split = rand_path.split('/')
            rand_img_path_split[4] = rand_img_path_split[4]+'_Snow'
            rand_img_path = "/".join(rand_img_path_split)
        ...
        if aug_type == 'cartoon':
            ...
            rand_img_path_split = rand_path.split('/')
            rand_img_path_split[4] = rand_img_path_split[4]+'_Cartoon'
            rand_img_path = "/".join(rand_img_path_split)
            rand_img = read_image(rand_img_path)
        ...
        if aug_type == 'FDA':
            ...
            rand_im_src = rand_img
            im_trg = style_img

            rand_im_src = rand_im_src.resize( (224,224), Image.BICUBIC )
            im_trg = im_trg.resize( (224,224), Image.BICUBIC )

            rand_im_src = np.asarray(rand_im_src, np.float32)
            im_trg = np.asarray(im_trg, np.float32)

            rand_im_src = rand_im_src.transpose((2, 0, 1))
            im_trg = im_trg.transpose((2, 0, 1))

            rand_src_in_trg = FDA_source_to_target_np( rand_im_src, im_trg, L=0.01 )

            rand_src_in_trg = rand_src_in_trg.transpose((1,2,0))
            rand_src_in_trg[rand_src_in_trg>255.0] = 255.0
            rand_src_in_trg[rand_src_in_trg<0] = 0
            rand_img = Image.fromarray(rand_src_in_trg.astype(np.uint8))
        ...
        if aug_type == 'edged':
            ...
            rand_img_path_split = rand_path.split('/')
            rand_img_path_split[2] = 'office-home-edged'
            rand_img_path = "/".join(rand_img_path_split)
            rand_img = read_image(rand_img_path)
        ...
        if self.transform is not None:
            if aug_type=='adain' or aug_type=='style':
                ...
                rand_img = self.adain_transform(rand_img)
            else:    
    ...
    def __getitem__(self, index):
        ...
        if isinstance(img_path,tuple):
            ...
            img = read_image(img_path)
            aug_index = random.randint(0, 5) if self.training==False else index%6
            if self.transform is not None:
                img = self.transform(img)
            ...
            for i, rand_img_path in enumerate(rand_img_paths):
                ...
                if self.transform is not None:
                    rand_img = self.transform(rand_img)
                rand_aug_img, aug_type = self.get_aug_full_img(rand_indices[i], aug_index)
                rand_aug_imgs.append(rand_aug_img)
            #torchvision.utils.save_image(img, f'shuffled/{index}.png')  
            # img_path = img_path.replace('OfficeHomeDataset', 'office-home-aug')
            # img_path_spilt = img_path.split('.')
            # img_path_spilt[-2]+=f'_{aug_type}'
            # img_path = ".".join(img_path_spilt)
            # dir_path = "/".join(img_path.split('/')[:-1])
            # if not osp.isdir(dir_path):
            #     os.mkdir(dir_path)         
            return rand_aug_imgs, aug_img, img, aug_type, pid, camid, trackid, img_path,idx
