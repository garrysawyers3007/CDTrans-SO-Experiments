...
class build_transformer(nn.Module):
    ...
    def forward(self, aug_x, x, label=None, cam_label= None, view_label=None, return_logits=False):  # label is unused if self.cos_layer == 'no'
        global_feat = self.base(aug_x, x, cam_label=cam_label, view_label=view_label)
        feat = self.bottleneck(global_feat)
        if return_logits:
            if self.cos_layer:
                cls_score = self.arcface(feat, label)
