...
def do_train_pretrain(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, dom_cls=False):
    ...
    for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader_shuffled):
        with torch.no_grad():
            img = img.to(device)
            camids = camids.to(device)
            target_view = target_view.to(device)
    ...
    for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(train_loader_clean):
        with torch.no_grad():
            img = img.to(device)
            camids = camids.to(device)
            target_view = target_view.to(device)
...
def do_inference(cfg,
        ...
        with torch.no_grad():
            ...
            if cfg.TEST.EVAL:
                if cfg.MODEL.TASK_TYPE == 'classify_DA':
                    ...
                    for t, p in zip(torch.tensor(pid).view(-1), preds.view(-1)):
    ...
    print("Per class acc:", confusion_matrix.diag()/confusion_matrix.sum(1))
