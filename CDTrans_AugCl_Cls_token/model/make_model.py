...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory):
        ...
        if self.ID_LOSS_TYPE == 'arcface':
            ...
            self.classifier = nn.Linear(2*self.in_planes, self.num_classes, bias=False)
            self.classifier.apply(weights_init_classifier)

        self.bottleneck = nn.BatchNorm1d(2*self.in_planes)
        self.bottleneck.bias.requires_grad_(False)
        self.bottleneck.apply(weights_init_kaiming)
    ...
    def forward(self, aug_x, x, label=None, cam_label= None, view_label=None, return_logits=False, cls_token=None):  # label is unused if self.cos_layer == 'no'
        global_feat = self.base(aug_x, x, cam_label=cam_label, view_label=view_label, cls_token=cls_token)
        feat = self.bottleneck(global_feat)
        if return_logits:
            if self.cos_layer:
                cls_score = self.arcface(feat, label)
