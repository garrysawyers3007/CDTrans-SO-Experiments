...
class Attention(nn.Module):
    ...
    def forward(self, aug_x, x, key):
        ...
        if key is None:
            ...
            cls_k = key[:, :, 0].unsqueeze(-2).repeat(1,1,N,1)
            attn = (q @ cls_k.transpose(-2, -1)) * self.scale
...
class Block(nn.Module):
    ...
    def forward(self, aug_x,  x, key=None):
        x = x + self.drop_path(self.attn(self.norm1(aug_x), self.norm1(x), key))
...
class TransReID(nn.Module):
    ...
    def forward_features(self, aug_x, x, camera_id, view_id, cls_token=None):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_tokens, x), dim=1)
        aug_x = torch.cat((cls_tokens, aug_x), dim=1)

        x = x + self.pos_embed
        x = self.pos_drop(x)

        aug_x = aug_x + self.pos_embed
        ...
        if self.local_feature:
            ...
            for blk in self.blocks:
                x = blk(x, x)
            x = self.norm(x)
            return torch.cat([x[:, 0], cls_token], dim=1)

    def forward(self, aug_x, x, cam_label=None, view_label=None, cls_token=None):
        x = self.forward_features(aug_x, x, cam_label, view_label, cls_token)
        return x
