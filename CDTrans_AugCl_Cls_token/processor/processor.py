...
def do_train_pretrain(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, patch_size, dom_cls=False):
    ...
    model_augcl.base.norm.register_forward_hook(get_features(f'feats'))
    features = {}
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (rand_aug_img, aug_img, img, aug_type, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            img = img.to(device)
            final_rand_aug_img = final_rand_aug_img.to(device)
            target = vid.to(device)
            target_cam = target_cam.to(device)
            target_view = target_view.to(device)
            with torch.no_grad():
                _ = model_augcl(final_rand_aug_img, final_rand_aug_img, target, cam_label=target_cam, view_label=target_view)
                cls_token = features['feats'][:,0]
            with amp.autocast(enabled=True):
                score, feat = model(aug_img, aug_img, target, cam_label=target_cam, view_label=target_view, cls_token=cls_token)
            ...
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (rand_img, img, vid, camid, camids, target_view, _) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        camids = camids.to(device)
                        target_view = target_view.to(device)
                        _ = model_augcl(img, img, target, cam_label=target_cam, view_label=target_view)
                        cls_token = features['feats'][:,0]
                        output_prob = model(img, img, cam_label=camids, view_label=target_view, return_logits=True, cls_token=cls_token)
    ...
    for n_iter, (rand_img, img, vid, camid, camids, target_view, _) in enumerate(val_loader):
        with torch.no_grad():
            ...
            _ = model_augcl(img, img, target, cam_label=target_cam, view_label=target_view)
            cls_token = features['feats'][:,0]
            feat = model(img, img, cam_label=camids, view_label=target_view, return_logits=True, cls_token=cls_token)
            if cfg.MODEL.TASK_TYPE == 'classify_DA':
