...
class build_transformer(nn.Module):
    ...
    def forward(self, aug_x, x, label=None, cam_label= None, view_label=None, return_logits=False, layer_num=0):  # label is unused if self.cos_layer == 'no'
        global_feat, key = self.base(aug_x, x, cam_label=cam_label, view_label=view_label, layer_num=layer_num)
        feat = self.bottleneck(global_feat)
        if return_logits:
            if self.cos_layer:
                cls_score = self.arcface(feat, label)
            ...
            return cls_score, global_feat, key  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return feat
