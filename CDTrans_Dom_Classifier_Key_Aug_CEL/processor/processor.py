...
def do_train_pretrain(cfg,
    ...
    fc_layer = nn.Linear(197*12*64, 6)
    fc_layer.cuda()
    ...
    model.base.blocks[layer_num-1].attn.qkv.register_forward_hook(get_features('feats'))
    features = {}
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (rand_aug_img, aug_img, img, aug_type, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            rand_inputs_style_for_adain=inputs_style[:rand_aug_img_adain.size(0)].cuda()
            try:
                with torch.no_grad():
                    ...
                    rand_data_stylized_adain = style_transfer(vgg, decoder, rand_aug_img_adain, rand_inputs_style_for_adain, 0.5)
                    rand_aug_im_restyled_styleaug = augmentor(rand_aug_img_styleaug)
                ...
                rand_data_stylized = normalize(torch.cat([rand_data_stylized_adain, rand_aug_im_restyled_styleaug], 0))

                rand_aug_img = torch.cat((rand_aug_img_others, rand_data_stylized), dim=0)
            ...
            with amp.autocast(enabled=True):
                score, feat, key = model(aug_img, aug_img, target, cam_label=target_cam, view_label=target_view, layer_num=layer_num)
                loss = loss_fn(score, feat, target, target_cam)
                #key = features['feats'].reshape(img.shape[0], 197, 3, 12, 64).permute(2, 0, 3, 1, 4)[1]

            aug_output = fc_layer(torch.flatten(key, start_dim=1).float())
            aug_labels = torch.tensor(np.vectorize(aug_dict.get)(aug_type)).to(device)
            loss_cel = nn.CrossEntropyLoss()(aug_output, aug_labels)
            loss+=loss_cel
            ...
            fc_layer_acc+=(aug_output.max(1)[1] == aug_labels).float().mean()
        ...
        end_time = time.time()
        print("Aug Cl acc:", fc_layer_acc.item()/len(train_loader))
            ...
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (rand_img, img, vid, camid, camids, target_view, _) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        camids = camids.to(device)
                        target_view = target_view.to(device)
                        output_prob = model(img, img, cam_label=camids, view_label=target_view, return_logits=True, layer_num=layer_num)
