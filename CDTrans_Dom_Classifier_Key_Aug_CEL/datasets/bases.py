...
class ImageDataset(Dataset):
    def __init__(self, dataset, transform=None, style_path=None, adain_transform=None, mode='RGB', patch_size=1):
    ...
    def get_aug_full_img(self, index, rand_index):

        path, pid, camid, trackid, idx = self.dataset[index]
        rand_path, rand_pid, rand_camid, rand_trackid, rand_idx = self.dataset[rand_index]
    ...
    def __getitem__(self, index):
        ...
        if isinstance(img_path,tuple):
            ...
            if self.transform is not None:
                img = self.transform(img)
                rand_img = self.transform(rand_img)
            aug_img, rand_aug_img, aug_type = self.get_aug_full_img(index, rand_index)
            ...
            return rand_aug_img, aug_img, img, aug_type, pid, camid, trackid, img_path.split('/')[-1],idx
