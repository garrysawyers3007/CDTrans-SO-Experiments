...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            labels = data[1]
            inputs = inputs.cuda()
            outputs = netC(netB(netF(inputs, inputs)))
            if start_test:
...
def train_target(args):
    ...
    rare_classes_dict = {
        'Art2Clipart': [20, 14, 11, 36],
        'Art2Product': [56, 4, 24, 20, 31, 17],
        'Art2Real_World': [17, 20],
        'Clipart2Art': [14, 22, 34, 52],
        'Clipart2Product': [64, 28, 10, 51],
        'Clipart2Real_World': [23, 17, 45, 5, 63]
    }

    rare_classes = rare_classes_dict[args.configuration]
    ...
    ## set base network
    model = make_model(cfg, num_class=65, camera_num=1, view_num = 1)
    modelpath = args.output_dir_src+'/transformer_best_model.pth'
    model.load_state_dict(torch.load(modelpath))
    model.cuda()

    model_logit = make_model(cfg, num_class=65, camera_num=1, view_num = 1)
    modelpath = args.feature_path+'/transformer_best_model.pth'
    model_logit.load_state_dict(torch.load(modelpath))
    for k,v in model_logit.named_parameters():
    ...
    netF = model.base
    netB = model.bottleneck
    netC = model.classifier
    ...
    loss_func, center_criterion = make_loss(cfg, num_classes=65)

    optimizer, _ = make_optimizer(cfg, model, center_criterion)
    # optimizer = op_copy(optimizer)
    # optimizer_c, _ = make_optimizer(cfg, netC, center_criterion)
    # optimizer_c = op_copy(optimizer_c)
    
    scheduler = create_scheduler(cfg, optimizer)
    # scheduler_c = create_scheduler(cfg, optimizer_c)

    #building feature bank and score bank
    loader = dset_loaders["target"]
    num_sample=len(loader.dataset)
    fea_bank=torch.randn(num_sample,768)
    score_bank = torch.randn(num_sample, 65).cuda()
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            #labels = data[1]
            inputs = inputs.cuda()
            output = netB(netF(inputs, inputs))
    ...
    epoch_break = len(dset_loaders["target"])
    iter_num = 0
    ...
    while iter_num < max_iter:
        try:
            inputs_test, target, tar_idx = iter_test.next()
        except:
        ...
        if iter_num % epoch_break == 0:
            scheduler.step(curr_epoch)
            curr_epoch+=1

        optimizer.zero_grad()
        features_test = netB(netF(inputs_test, inputs_test))
        outputs_test = netC(features_test)
        softmax_out = nn.Softmax(dim=1)(outputs_test)
        #output_re = softmax_out.unsqueeze(1)

        indices = torch.zeros(target.shape).cuda()
        target = target.cuda()
        for cls in rare_classes:
            indices+= target==cls
        indices = torch.nonzero(indices, as_tuple=True)[0]
        
        score_logit = model_logit(inputs_test, inputs_test, target, return_logits=True)
        score_logit=score_logit.detach()
        logit_loss = nn.MSELoss()(outputs_test[indices], score_logit[indices])
        logit_loss[logit_loss!=logit_loss]=0
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            writer.add_scalar("Accuracy", acc_s_te, iter_num)
...
if __name__ == "__main__":
    ...
    if args.dset == 'OfficeHomeDataset':
