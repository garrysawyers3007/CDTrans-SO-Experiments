...
def do_train_pretrain(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (rand_aug_img, aug_img, img, aug_type, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            rand_inputs_style_for_adain=inputs_style[:rand_aug_img_adain.size(1)].cuda()
            try:
                with torch.no_grad():
                ...
                aug_type = np.concatenate((aug_type[idx_others], aug_type[idx_adain], aug_type[idx_styleaug]), axis=0)
            ...
            with amp.autocast(enabled=True):
                score, feat, _ = model(final_rand_aug_img, final_rand_aug_img, target, cam_label=target_cam, view_label=target_view, layer_num=layer_num)
                loss = loss_fn(score, feat, aug_labels, target_cam)
            ...
            if isinstance(score, list):
                acc = (score[0].max(1)[1] == aug_labels).float().mean()
            else:
                acc = (score.max(1)[1] == aug_labels).float().mean()
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                torch.cuda.empty_cache()
                model.eval()
                style_loader2 = DataLoader(style_data, batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=True, num_workers=cfg.DATALOADER.NUM_WORKERS, drop_last=True)
                for n_iter, (rand_img, aug_img, img, aug_type, vid, camid, camids, target_view, path) in enumerate(val_loader):
                    try:
                        inputs_style = iter_source_style_2.next()
                    except:
                        iter_source_style_2 = iter(style_loader2)
                        inputs_style = iter_source_style_2.next()
                    ...
                    try:
                        ...
                        target_view = torch.cat((target_view_others, target_view_adain, target_view_styleaug), dim=0)
                        aug_type = np.concatenate((aug_type[idx_others], aug_type[idx_adain], aug_type[idx_styleaug]), axis=0)
                        path = np.concatenate((path[idx_others], path[idx_adain], path[idx_styleaug]), axis=0)
                        ...
                        target_view = target_view_others
                        aug_type = aug_type[idx_others]
                        path = path[idx_others]

                    # for k in range(aug_img.shape[0]):
                    #     torchvision.utils.save_image(aug_img[k], f'data/office-home-aug/Clipart/{aug_type[k]}/{path[k]}')
                    ...
                    with torch.no_grad():
                        ...
                        camids = camids.to(device)
                        target_view = target_view.to(device)
                        aug_labels = torch.tensor(np.vectorize(aug_dict.get)(aug_type))

                        output_prob = model(aug_img, aug_img, cam_label=camids, view_label=target_view, return_logits=True)
                        evaluator.update((output_prob, aug_labels))
    ...
    for n_iter, (rand_img, aug_img, img, aug_type, vid, camid, camids, target_view, _) in enumerate(val_loader):
        try:
            inputs_style = iter_source_style.next()
        except:
            iter_source_style = iter(style_loader)
        ...
        with torch.no_grad():
            ...
            target_view = target_view.to(device)
            aug_labels = torch.tensor(np.vectorize(aug_dict.get)(aug_type))
            feat = model(aug_img, aug_img, cam_label=camids, view_label=target_view, return_logits=True)
            if cfg.MODEL.TASK_TYPE == 'classify_DA':
                evaluator.update((feat, aug_labels))
            else:
                evaluator.update((feat, vid, camid))
    if cfg.MODEL.TASK_TYPE == 'classify_DA':
    ...
    evaluator.reset()
    style_loader = DataLoader(style_data, batch_size=cfg.TEST.IMS_PER_BATCH, shuffle=True, num_workers=cfg.DATALOADER.NUM_WORKERS, drop_last=True)
    for n_iter, (rand_img, aug_img, img, aug_type, vid, camid, camids, target_view, _) in enumerate(train_loader_clean):
        try:
            inputs_style = iter_source_style.next()
        except:
            iter_source_style = iter(style_loader)
        ...
        with torch.no_grad():
            ...
            target_view = target_view.to(device)
            aug_labels = torch.tensor(np.vectorize(aug_dict.get)(aug_type))
            feat = model(aug_img, aug_img, cam_label=camids, view_label=target_view, return_logits=True, layer_num=layer_num)
            if cfg.MODEL.TASK_TYPE == 'classify_DA':
                evaluator.update((feat, aug_labels))
            else:
                evaluator.update((feat, vid, camid))
