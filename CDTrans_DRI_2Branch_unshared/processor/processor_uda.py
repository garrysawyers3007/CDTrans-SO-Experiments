...
from datasets.bases import ImageDataset, ImageList_style
...
def do_train_uda(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, dom_weight):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (rand_aug_img, aug_img, imgs, aug_type, p, vid, target_cam, target_view, _) in enumerate(train_loader):
            img = rand_aug_img.to(device)
            t_img = aug_img.to(device) #target img
            target = vid.to(device)
            t_pseudo_target = vid.to(device)
            p = p.to(device)
            label_knn = vid.cuda()

            rand_aug_imgs = []
            for i in range(img.shape[1]):
                rand_aug_imgs.append(img[:,i])
            img = torch.stack(rand_aug_imgs, dim=0)
            ...
            img_adain = img[:, idx_adain[0]]
            t_img_adain = t_img[idx_adain[0]]
            target_adain = target[idx_adain[0]]
            t_pseudo_target_adain = t_pseudo_target[idx_adain[0]]
            label_knn_adain = label_knn[idx_adain[0]]
            p_adain = p[idx_adain[0]]

            img_styleaug = img[:, idx_styleaug[0]]
            t_img_styleaug = t_img[idx_styleaug[0]]
            target_styleaug = target[idx_styleaug[0]]
            t_pseudo_target_styleaug = t_pseudo_target[idx_styleaug[0]]
            label_knn_styleaug = label_knn[idx_styleaug[0]]
            p_styleaug = p[idx_styleaug[0]]

            img_others = img[:, idx_others]
            t_img_others = t_img[idx_others]
            target_others = target[idx_others]
            t_pseudo_target_others = t_pseudo_target[idx_others]
            label_knn_others = label_knn[idx_others]
            p_others = p[idx_others]

            inputs_style_for_adain=inputs_style[:t_img_adain.size(0)].cuda()
            rand_inputs_style_for_adain=inputs_style[:img_adain.shape[1]].cuda()

            try:
                with torch.no_grad():
                    data_stylized_adain = style_transfer(vgg, decoder, t_img_adain, inputs_style_for_adain, 0.5)
                    t_im_restyled_styleaug = augmentor(t_img_styleaug)

                    rand_data_stylized_adain = []
                    rand_aug_im_restyled_styleaug = []
                    for i in range(img_adain.shape[0]):
                        rand_data_stylized_adain.append(style_transfer(vgg, decoder, img_adain[i], rand_inputs_style_for_adain, 0.5))
                    rand_data_stylized_adain = torch.stack(rand_data_stylized_adain, dim=0)

                    for i in range(img_styleaug.shape[0]):
                        rand_aug_im_restyled_styleaug.append(augmentor(img_styleaug[i]))
                    rand_aug_im_restyled_styleaug = torch.stack(rand_aug_im_restyled_styleaug, dim=0)

                    data_stylized = normalize(torch.cat([data_stylized_adain, t_im_restyled_styleaug], 0))
                    rand_data_stylized = normalize(torch.cat([rand_data_stylized_adain, rand_aug_im_restyled_styleaug], 1))

                    img = torch.cat((img_others, rand_data_stylized), dim=1)
                    t_img = torch.cat((t_img_others, data_stylized), dim=0)
                    target = torch.cat((target_others, target_adain, target_styleaug), dim=0)
                    t_pseudo_target = torch.cat((t_pseudo_target_others, t_pseudo_target_adain, t_pseudo_target_styleaug), dim=0)
                    label_knn = torch.cat((label_knn_others, label_knn_adain, label_knn_styleaug), dim=0)
                    p = torch.cat((p_others, p_adain, p_styleaug), dim=0)
                    aug_type = np.concatenate((aug_type[idx_others], aug_type[idx_adain[0]], aug_type[idx_styleaug[0]]), axis=0)

            except:
                img = img_others
                t_img = t_img_others
                target = target_others
                t_pseudo_target = t_pseudo_target_others
                label_knn = label_knn_others
                p = p_others
                aug_type = aug_type[idx_others]

            W = int(t_img[0, 0].shape[0]/8)
            H = int(t_img[0, 0].shape[1]/8)

            final_rand_aug_img = torch.zeros((t_img.shape[0], 3, 224, 224)).to(device)
            for i in range(8):
                for j in range(8):
                    final_rand_aug_img[:, :, i*W:i*W+W,j*H:j*H+H] = img[random.randint(0, 3), :, :, i*W:i*W+W,j*H:j*H+H]
            
            img = shuffle_batch(final_rand_aug_img, 8)

            aug_labels = torch.tensor(np.vectorize(aug_dict.get)(aug_type)).to(device)
            optimizer.zero_grad()
            ...
            with amp.autocast(enabled=True):
                ...
                def contrastive_loss(output, p):
                    prob = F.softmax(output, dim=-1)
                    return -torch.mean(p * torch.sum(prob * torch.log(prob+1e-5), 1))
                ...
                loss1 = loss_fn(self_score1, self_feat1, aug_labels, target_cam)
                loss2 = loss_fn(score2, feat2, t_pseudo_target, target_cam)
                # loss3 = distill_loss(score_fusion, score2)
                loss3 = contrastive_loss(score2, p)
                loss = loss2 + dom_weight*loss3 #+loss1
            ...
            if isinstance(self_score1, list):
                acc = (self_score1[0].max(1)[1] == aug_labels).float().mean()
            else:
                acc = (self_score1.max(1)[1] == aug_labels).float().mean()
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (rand_img, img, vid, camid, camids, target_view, idex) in enumerate(val_loader):
                    with torch.no_grad():
                        img = img.to(device)
                        camids = camids.to(device)
                        target_view = target_view.to(device)
    ...
    if cfg.MODEL.TASK_TYPE == 'classify_DA':
        accuracy, _ = evaluator.compute()  
        logger.info("Classify Domain Adapatation Source Results - Best model")
        logger.info("Accuracy: {:.1%}".format(accuracy))
    else:
        cmc, mAP, _, _, _, _, _ = evaluator.compute()
        logger.info("Best Model Validation Results ")
