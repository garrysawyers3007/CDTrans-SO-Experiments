...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(img_path,tuple):
            ...
            if r>0.5:
                rand_aug_index = aug_index
            else:
                rand_aug_index = random.randint(0, 5)
                while rand_aug_index==aug_index:
                    rand_aug_index = random.randint(0, 5)
            img = read_image(img_path)
            if self.transform is not None:
                img = self.transform(img)
            ...
            for i, rand_img_path in enumerate(rand_img_paths):
                ...
                rand_aug_img, _ = self.get_aug_full_img(rand_indices[i], rand_aug_index)
            ...
            #['FDA','style','adain','weather','cartoon', 'clean']
            img_path = img_path.replace('OfficeHomeDataset', 'office-home-aug')
            if 'FDA' in img_path:
                pid=0
            if 'style' in img_path:
                pid=1
            if 'adain' in img_path:
                pid=2
            if 'weather' in img_path:
                pid=3
            if 'cartoon' in img_path:
                pid=4
            ...
            if r>0.5:
                ...
                p=-1           
            return rand_aug_imgs, aug_img, img, aug_type, p, pid, camid, trackid, img_path.split('/')[-1],idx
