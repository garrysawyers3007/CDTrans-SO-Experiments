...
class Attention_3_branches(nn.Module):
    ...
    def forward(self, x, x2, use_attn=True, inference_target_only=False):
        ...
        q2 = self.q2(x2).reshape(B, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        k2 = self.k2(x2).reshape(B, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        v2 = self.v2(x2).reshape(B, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        # q2, k2, v2 = qkv2[0], qkv2[1], qkv2[2]   # make torchscript happy (cannot use tensor as tuple)
...
class Block_3_branches(nn.Module):
    ...
    def forward(self, x, x2, x1_x2_fusion, use_cross=False, use_attn=True, domain_norm=False,inference_target_only=False):
        ...
        # else:
        xa_attn, xa_attn2, cross_attn = self.attn(self.norm1(x),self.norm1(x2), inference_target_only=inference_target_only)
        ...
        if self.local_feature:
            ...
            if self.block_pattern == '3_branches':
                ...
                for i, blk in enumerate(self.blocks):
                    x,x2, cross_attn = blk(x,x2,x1_x2_fusion,use_cross=self.use_cross,use_attn=self.use_attn, domain_norm=domain_norm, inference_target_only=inference_target_only)
