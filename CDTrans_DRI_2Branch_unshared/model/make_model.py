...
class build_transformer(nn.Module):
    ...
    def forward(self, aug_x, x, label=None, cam_label= None, view_label=None, return_logits=False, layer_num=0):  # label is unused if self.cos_layer == 'no'
        global_feat = self.base(aug_x, x, cam_label=cam_label, view_label=view_label, layer_num=layer_num)
        feat = self.bottleneck(global_feat)
        if return_logits:
            if self.cos_layer:
                cls_score = self.arcface(feat, label)
...
class build_uda_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory):
        ...
        if self.ID_LOSS_TYPE == 'arcface':
            ...
            self.classifier = nn.Linear(self.in_planes, self.num_classes, bias=False)
            self.classifier_dom = nn.Linear(self.in_planes, 6, bias=False)
            self.classifier.apply(weights_init_classifier)
    ...
    def forward(self, x, x2, label=None, cam_label= None, view_label=None, domain_norm=False, return_logits=False, return_feat_prob=False, cls_embed_specific=False):  # label is unused if self.cos_layer == 'no'
        ...
        if self.training or return_feat_prob:
            if self.ID_LOSS_TYPE in ('arcface', 'cosface', 'amsoftmax', 'circle'):
                cls_score = self.classifier_dom(feat, label) if self.training else None
                cls_score2 = self.classifier(feat2, label)
                cls_score3 = self.classifier(feat3, label) if self.training else None
            else:
                cls_score = self.classifier_dom(feat) if self.training else None
                cls_score2 = self.classifier(feat2)
