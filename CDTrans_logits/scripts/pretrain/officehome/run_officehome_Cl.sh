...
do
    ...
    python train.py --config_file configs/pretrain.yml --lin_prot_path '../logs/pretrain/deit_base/office-home/Clipart_2_'$target'_only_classifier/transformer_best_model.pth' --configuration 'Clipart2'$target MODEL.DEVICE_ID "('3')" DATASETS.NAMES 'OfficeHome' \
    OUTPUT_DIR '../logs/pretrain/'$model'/office-home/Clipart_2_'$target'_logits_2' \
    DATASETS.ROOT_TRAIN_DIR '../data/OfficeHomeDataset/Clipart.txt' \
    DATASETS.ROOT_TEST_DIR '../data/OfficeHomeDataset/'$target'.txt'   \
    MODEL.Transformer_TYPE $model_type \
    MODEL.PRETRAIN_PATH '../data/pretrainModel/'$pretrain_model \
