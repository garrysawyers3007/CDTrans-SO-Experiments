...
def do_train_pretrain(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, layer_num, model_logit, rare_classes):
    log_period = cfg.SOLVER.LOG_PERIOD
    checkpoint_period = cfg.SOLVER.CHECKPOINT_PERIOD
    eval_period = cfg.SOLVER.EVAL_PERIOD
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (aug_img, aug_type, img, feature, vid, target_cam, target_view, paths, _) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                score, feat = model(img, img, target, cam_label=target_cam, view_label=target_view)
                score_logit = model_logit(img, img, target, cam_label=target_cam, view_label=target_view, return_logits=True)
                score_logit=score_logit.detach()
                logit_loss = nn.MSELoss()(score[indices], score_logit[indices])
                loss = loss_fn(score, feat, target, target_cam)
...
def do_train_pretrain_patch_augs(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, num_patch_wise, layer_num, model_logit, rare_classes):
    log_period = cfg.SOLVER.LOG_PERIOD
    checkpoint_period = cfg.SOLVER.CHECKPOINT_PERIOD
    eval_period = cfg.SOLVER.EVAL_PERIOD
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (aug_img, aug_type, img, vid, target_cam, target_view, paths, _) in enumerate(train_loader):
            ...
            for i in range(num_patch_wise):
                final_patch_aug_img = torch.zeros((aug_img.shape[0], 3, 224, 224))
                for i in range(int(W)):
                    for j in range(int(H)):
                        aug_type = random.choice(aug_list)
                        #print(i*16, i*16+16, j*16, j*16+16)
                        final_patch_aug_img[:, :, i*patch_size:i*patch_size+patch_size,j*patch_size:j*patch_size+patch_size] = aug_img[:, aug_type, :, i*patch_size:i*patch_size+patch_size, j*patch_size:j*patch_size+patch_size]
