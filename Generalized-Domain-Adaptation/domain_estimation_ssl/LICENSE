...
from trans_models import make_model
...
class SimCLR(object):
    ...
    def train(self):
        ...
        if self.config.dataset.parent == "Digit":
            ...
            elif self.config.model.base_model == 'deit_base':
                cfg.merge_from_file('./config/pretrain.yml')
                cfg.freeze()
                model = make_model(cfg, num_class=31, camera_num=1, view_num = 1).to(self.device)
            else:
