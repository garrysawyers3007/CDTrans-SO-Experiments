...
def get_dataset(parent, dset_taple, domain_label, transform=None, use_jigsaw=False, grid=3):
    ...
    if parent == 'Digit':
        ...
        text_train = os.path.join("../../data", parent, "{}_list.txt".format(dset_name))
        if parent=='Visda':
            resize=(160,160)
        elif parent == 'PACS':
            resize=(228,228)
    ...
    filenames = np.array(["/".join([filesplit for i, filesplit in enumerate(filename.split('/')) if i!=2]) for filename in filenames])
    ...
    root = os.path.join("../../data", parent, "images")
