...
for target in 'Product' 'Real_World' 'Clipart'
do
    ...
    python train.py --config_file configs/pretrain.yml --freeze_MHSA True --augs True MODEL.DEVICE_ID "('0')" DATASETS.NAMES 'OfficeHome' \
    OUTPUT_DIR '../logs/pretrain/'$model'/office-home/Art_2_'$target'_freeze_MHSA_augs' \
    DATASETS.ROOT_TRAIN_DIR './data/OfficeHomeDataset/Art.txt' \
    DATASETS.ROOT_TEST_DIR './data/OfficeHomeDataset/'$target'.txt'   \
    MODEL.Transformer_TYPE $model_type \
    MODEL.PRETRAIN_PATH './data/pretrainModel/'$pretrain_model \
