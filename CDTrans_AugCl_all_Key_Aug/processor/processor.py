...
def do_train_pretrain(cfg,
    ...
    for layer_num in range(12):
        model_augcl.base.blocks[layer_num].attn.qkv.register_forward_hook(get_features(f'feats_{layer_num}'))
    features = {}
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (rand_aug_img, aug_img, img, aug_type, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                for i in range(12):
                    keys.append(features[f'feats_{i}'].reshape(img.shape[0], 197, 3, 12, 64).permute(2, 0, 3, 1, 4)[1])
            with amp.autocast(enabled=True):
                score, feat = model(aug_img, aug_img, target, cam_label=target_cam, view_label=target_view, keys=keys)
