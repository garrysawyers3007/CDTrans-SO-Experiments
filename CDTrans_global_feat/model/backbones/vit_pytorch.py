...
class Attention(nn.Module):
    ...
    def forward(self, aug_x, x):
        B, N, C = x.shape
        rand = torch.ones(B, N, C).cuda()
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_aug = self.qkv(aug_x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_rand = self.qkv(rand).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class Block(nn.Module):
    ...
    def forward(self, aug_x, x):
        x = x + self.drop_path(self.attn(self.norm1(aug_x), self.norm1(x)))
...
class TransReID(nn.Module):
    ...
    def forward_features(self, aug_x, x, feature, camera_id, view_id, layer_num=-1):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        if self.local_feature:
            ...
            for i, blk in enumerate(self.blocks):
                if i==0:
                    ...
                    x = blk(x, x)
                if i==layer_num-1 and feature is not None:
                    x = 0.1*feature+0.9*x #change here!!!!
            x = self.norm(x)
            return x[:, 0], x

    def forward(self, aug_x, x, cam_label=None, view_label=None, feature=None, layer_num=-1):
        x = self.forward_features(aug_x, x, feature, cam_label, view_label, layer_num=layer_num)
        return x
