...
class build_transformer(nn.Module):
    ...
    def forward(self, aug_x, x, label=None, cam_label= None, view_label=None, return_logits=False, feature=None, layer_num=-1):  # label is unused if self.cos_layer == 'no'
        global_feat, feature = self.base(aug_x, x, cam_label=cam_label, view_label=view_label, feature=feature,layer_num=layer_num)
        feat = self.bottleneck(global_feat)
        if return_logits:
            if self.cos_layer:
                cls_score = self.arcface(feat, label)
            ...
            return cls_score, global_feat, feature  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return feat, feature
